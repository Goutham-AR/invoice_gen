...
##**IT102/IT104 Elements**
 
Both IT102 (Quantity) and IT104 (Price) write a string of zeros to the 4th position right of the decimal _(ie '6.000' = 6)._
 
Recent changes confirmed by the service provider indicate:
 
- Line-Item Quantity values (IT102) are no longer written as a decimal number
- Line-Item Price values (IT104) can be written exponentially with decimal number values to the hundredth or 
     thousandth
 
  
 
**PO402 Element for alternative line-item cost verification**
 
The Encompass accounting system allows for line-item cost (IT104) values to be written to the hundredth or thousandth position right of the decimal, which requires rounding to produce a dollar/double value.   
 
As a workaround for this issue, the Encompass 810 provides a 2nd element under the PO4 segment mapped in the schema below reflecting a Double as value line-item cost against which to reconcile calculated line-item cost.
 
The PO402 element provides the IT1 cost total calculated by the Encompass system and invoiced to the customer.
 
~~**Cost calculation alternatives w/broken case values in IT102/IT104**
 
***The following calculation logic can be considered when IT104 (cost) contains an extended or exponential decimal value:***
 
 
**Method 1 (easiest, but not recommended):** 
1. Load IT102/IT104 as provided in the data
2. Add Line-Item SAC sum to PO402 
3. Write resulting sum as extended cost
 
**Method 2:**
 
1. Calculate IT1 sum using the standard method (IT102*IT104)~~
2. Compare the product of IT102*IT104 against PO402~~
- If the 2 numbers match, proceed with standard extendedcost calculation~~
- If the 2 numbers do not match, use PO402 value in place of IT102*IT104 product as the basis for extendedCost calculation
 
**Method 3 (currently used in Legacy):**
 
If TDS amount doesn't match with calculated cost of line items, then we add difference as charge rounding to line-item cost so that total matches.
##**Charted File Schema**
...
| **PO4** |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- |
| Name                    | Position | Min/Max | Data Type  | Field Comments  | Example  | Entity Col     |
| Pack (PO4)              |          |         |            |                 | PO4*     |                |
| Packs Per Case          | 1 | 12 | Numeric    | Implied decimal | 12*      | unitsPerPack   |
| Line-Item Cost | 2 |   | Double |  | 44.55 |  |
