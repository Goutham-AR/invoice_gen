...
**96 + 48 - 48 = QTY 1/96 Rate net to load as single line item**
...
**96 - 48 = QTY 1/48 Rate net to load as single line item**
...
**QTY 1/96 Rate net to load as single line item into Databricks**
 
 
*Scenario 4: Zero QTY and Zero Rate Line Items*
 
All line items containing 0 Rate values should be omitted from load.
...
There are occasionally unsupported extra columns added, without corresponding data. These columns should be omitted from ingestion.
...
Load both Product Description and item number from the either the *Memo/Description* or *Product/Service* column as outlined in the charted schema below.
 
The file will contain either the *Memo/Description* or *Product/Service* to capture this information, but both columns will not be present in the same file (either one or the other will be written).
 
##**Customer Name and Customer Number**
Load both Customer Name and Customer Number from the *Name* column as outlined in the charted schema below.
...
|Invoice Header   |Name               |Customer Name                              |Alphanumeric                |Fogo de Chao - Brea CA           |Optional              |shipTo.name          |invoice_summary.SHIP_TO_NAME            |                                      |
|Invoice Header   |Name               |Customer Number                            |Alphanumeric                |Fogo de Chao - Brea CA           |Mandatory             |vendorStoreId        |invoice_summary.SHIP_TO_VENDOR_STORE_ID |                                      |
|Invoice Header   |Due Date           |Invoice Due Date                           |M/DD/YYYY                   |3/15/2005                        |Optional              |invoiceDueDate       |invoice_summary.TERM_NET_DUE_DATE       |Pull from Col 3 if null or col omitted|
...
|Line Item Details|Product/Service    |Vendor Product Number                      |Alphanumeric                |Chelsea B. Drugstore Mixed 6 pack|Mandatory             |vendorProductNumber  |invoice_line_items.VENDOR_PRODUCT_NUMBER|                                      |
|Line Item Details|Product/Service    |Product Description                        |Alphanumeric                |Chelsea B. Drugstore Mixed 6 pack|Mandatory             |description          |invoice_line_items.ITEM_DESCRIPTION     |                                      |
|Line Item Details|Memo/Description   |Vendor Product Number                      |Alphanumeric                |Chelsea B. Drugstore Mixed 6 pack|Mandatory             |vendorProductNumber  |invoice_line_items.VENDOR_PRODUCT_NUMBER|                                      |
|Line Item Details|Memo/Description   |Product Description                        |Alphanumeric                |Chelsea B. Drugstore Mixed 6 pack|Mandatory             |description          |invoice_line_items.ITEM_DESCRIPTION     |                                      |
|Line Item Details|Rate               |Unit Price                                 |Numeric                     |2.73                             |Mandatory             |cost                 |invoice_line_items.ITEM_COST            |                                      |

For one invoice in the first line only the Transaction Type, Date, Num, Name, Due Date should be given. For the rest of the line, line Item Details is only needed.
In the lines with Line Item Details, one tab space should be left for the invoice details that are not to be entered in that line. In the line with invoice details one tab space should be let for line items details file which are not to be entered in that line.
