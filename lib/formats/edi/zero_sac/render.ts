...
import type { ZeroSacInvoice, ZeroSacLineItem } from "./schema";
...
function it1Segment(li: ZeroSacLineItem, lineNumber: number): string {
...
function po4Segment(li: ZeroSacLineItem): string | undefined {
  if (li.packsPerCase === undefined) return undefined;
  return buildSegment("PO4", [String(li.packsPerCase)]);
}

function lineItemSegments(li: ZeroSacLineItem, lineNumber: number): string[] {
...
function invoiceTransactionSet(invoice: ZeroSacInvoice, controlNumber: string): string[] {
  const body: string[] = [
    bigSegment({
...
export function renderZeroSac(invoices: ZeroSacInvoice[]): string {
